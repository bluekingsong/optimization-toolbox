...
double GradientCalc::operator()(const Real *w, Real * g){
        ...
        if(fitIntercept)    g[data->n - 1] = 0; // don't reguarize the intercept term
        //std::cout<<"GradientCalc:g0Norm="<<vec_dot(g,g,data->n)<<" wNorm="<<vec_dot(w,w,data->n)<<std::endl;
        double NLL = 0.5 * l2 * vec_dot(w,w,data->n);  // fix bug, should count regularization ||W||^2
        if(fitIntercept)    NLL -= 0.5 * l2 * w[data->n - 1] * w[data->n - 1];
        for(uint32_t i = 0; i < data->l; ++i){
            ...
            if(fitIntercept)    t += w[data->n - 1];
            double ui = 1.0 / (1.0 + std::exp(-t));
            j=0;
            while(-1 != instance[j].index){
                g[instance[j].index] += (ui - yi) * instance[j].value; // / data->l; // gradient, X'*(U-Y)
                ++j;
            }
            if(fitIntercept)    g[data->n -1 ] += (ui - yi); //  / data->l;
            predictedProb[i] = ui;
            // preventing overflow
            if(yi)    t < -25 ? NLL -= t : NLL -= log(ui);
            else      t > 25  ? NLL += t : NLL -= log(1-ui);
        }
        funcVal = NLL;
        return NLL;
}
