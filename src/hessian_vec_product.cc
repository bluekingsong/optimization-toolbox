...
void HessianVecProduct::operator()(const Real *x,const uint32_t n,Real *y)const{
    ...
    uint32_t blockSize = problemData.l / sampleSize;
    //std::cout<<"block-size="<<blockSize<<" sampleSize="<<sampleSize<<std::endl;
    for(uint32_t i = 0; i < problemData.l; i += blockSize){
        ...
        while (feature->index >= 0){
            y[feature->index] += feature->value * dot * p * (1-p) * (problemData.l / sampleSize);
            feature += 1;
        }
    }
};
